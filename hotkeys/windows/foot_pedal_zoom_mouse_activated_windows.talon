os: windows
tag: talon_plugins.eye_zoom_mouse.zoom_mouse_activated
and tag: talon_plugins.eye_zoom_mouse.zoom_mouse_use_pedal
-
#left pedal
#middle 
key(pause): talon_plugins.eye_zoom_mouse.right_click()
key(keypad_6): talon_plugins.eye_zoom_mouse.right_click()

#rightt
key(scroll_lock): talon_plugins.eye_zoom_mouse.double_click()
key(keypad_7): talon_plugins.eye_zoom_mouse.double_click()

#right pedal
#left
key(pageup): talon_plugins.eye_zoom_mouse.triple_click()
key(keypad_8): talon_plugins.eye_zoom_mouse.triple_click()
#middle
key(pagedown): talon_plugins.eye_zoom_mouse.mouse_drag()
key(keypad_9): talon_plugins.eye_zoom_mouse.mouse_drag()

key(ctrl-shift-f12): talon_plugins.eye_zoom_mouse.mouse_move()

#Blue2 
key(home): talon_plugins.eye_zoom_mouse.right_click()
key(insert): talon_plugins.eye_zoom_mouse.mouse_trigger()
